...
public final class ContributionBridge implements AutoCloseable {
    private final Object monitor = new Object();
    private final Map<ContributionId, Entry<?, ?, ?>> entries = new LinkedHashMap<>();
    private final Sinks.Many<ContributionSnapshot> snapshots =
        Sinks.many().replay().latest();
    private long revision;
    private boolean closed;

    public ContributionBridge() {
        snapshots.tryEmitNext(new ContributionSnapshot(0, java.util.List.of()));
    }
    ...
    public <D, I, O> Mono<O> invoke(Context caller, ContributionKind<D, I, O> kind,
        ...
        synchronized (monitor) {
            var raw = entries.get(id);
            if (raw == null || raw.kind != kind || !raw.accepting) {
    ...
    public Flux<ContributionSnapshot> snapshots() {
        return snapshots.asFlux();
    }

    public Mono<Void> closeAsync() {
        java.util.List<Entry<?, ?, ?>> current;
        synchronized (monitor) {
            if (closed) {
                return Mono.empty();
            }
            closed = true;
            current = java.util.List.copyOf(entries.values());
        }
        return Flux.fromIterable(current).flatMap(this::revoke).then()
            .doOnSuccess(ignored -> snapshots.tryEmitComplete());
    }
    ...
    private List<Registration> registerAllNow(
        ...
        synchronized (monitor) {
            if (closed) {
                throw new IllegalStateException("contribution bridge is closed");
            }
    ...
    private void publishUnsafe() {
        revision++;
        snapshots.tryEmitNext(snapshotUnsafe());
    }

    private ContributionSnapshot snapshotUnsafe() {
        var values = entries.values().stream()
            ...
            .toList();
        return new ContributionSnapshot(revision, values);
    }

    private static final class Entry<D, I, O> {
