...
- [ ] 根 50 模块 `clean verify`、篡改/路径/依赖/脱敏安全用例、发行 ZIP、三轮可复现制品比较和最终仓外消费者均已通过；本地日志 `/private/tmp/v1-verify-reproducible-release.log` 与 `/private/tmp/v1-verify-distribution-local-m2.log`。Linux [GitHub Actions #35](https://github.com/sunker0115/fibra/actions/runs/34880038160) 曾在 `main` 合并提交 `7a24be5` 上通过全部四项门禁，但后续同源码树 #36 再次命中启动结果投影竞态。当前确定性回归已先红后绿，Engine 178/178、根 50 模块 `clean verify` 通过；仍需取得包含该修复的同一 HEAD Linux CI。
