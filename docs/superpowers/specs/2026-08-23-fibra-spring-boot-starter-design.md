...
上游运行时权威源：[`2026-08-24-fibra-engine-architecture.md`](2026-08-24-fibra-engine-architecture.md)。本文只冻结 Spring Framework 与 Spring Boot 接缝，不重复定义 loader、reconcile 或 deployment 算法。

## 1. 目标与系统边界

`fibra-spring` 是 `fibra-engine` 的 Spring Framework 生命周期适配；`fibra-spring-boot-autoconfigure` 把该适配接入 Spring Boot，`fibra-spring-boot-starter` 是推荐依赖入口。engine 独占 root、两个 loader、reconcile、watch source、readiness 和关闭；Spring 三模块不得复制这些算法。CLI、Web、Spring AI 或 Java DeepSeek Harness 都只是上层消费者，不得反向定义通用语义。
...
Spring Boot 还明确建议大型项目把通用 Spring 支持从自动配置中分离。因此新增 `fibra-spring` 承载 `SmartLifecycle` 和显式服务桥接；实际生命周期协调属于 `fibra-engine`，Boot 属性绑定和条件装配仍只在 autoconfigure。
...
| starter + autoconfigure 两层，通用运行时也放 autoconfigure | 拒绝 | 可复用的 Spring Framework 生命周期被 Boot 属性和条件装配绑死 |
| Spring、autoconfigure、starter 三层并由单一 runtime 统筹 | 采用 | 模块职责清楚，普通 Spring 与 Boot 共用同一启动、回滚和关闭实现 |
...
- 保存 `FibraSpringLifecycle` 和 `FibraServiceBridge`；
- 只使用 Spring Framework `SmartLifecycle`，不出现 Boot 注解、属性绑定、条件装配或注册资源；
- lifecycle 只委托 `FibraEngine.start()`/`close()`，不持有 loader 或 watcher；
- 不包含 Web、Shell、Spring AI、Actuator 或宿主业务。

`fibra-spring-boot-autoconfigure`：

- 保存 `FibraAutoConfiguration`、`FibraProperties`、完整属性校验、配置元数据处理器和 `AutoConfiguration.imports`；
...
### 5.1 `FibraSpringLifecycle`

`fibra-spring` 的公共 `SmartLifecycle`，构造器只接收一个 `FibraEngine`。`start()` 委托 engine start，`stop()`/`stop(Runnable)` 委托 engine close 并保证 callback 恰好一次。它不接收路径、watcher、loader 或 Boot properties。

普通 Spring 宿主显式声明 `FibraEngine`、`FibraSpringLifecycle` 和按需的 `FibraServiceBridge` bean；生命周期实现不复制 engine 的启动失败回滚和关闭顺序。

### 5.2 `FibraAutoConfiguration`

公共类名，用 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` 注册。bean 方法与内部协作类型不是公共 API。

默认自动配置是不可拆分的托管单元：只有当宿主不存在 `Context` 且不存在 `FibraEngine` 时才创建完整 engine，再把其只读资源视图和 lifecycle 暴露为 Spring bean。若宿主已经提供两者之一，整个托管运行时退让；宿主必须显式装配和关闭，不把宿主资源与框架的部分资源拼成未知所有权图。

这一规则刻意替代“每个 bean 都单独 `@ConditionalOnMissingBean`”。后者会让 lifecycle 无法判断一个资源应由谁关闭，并可能与宿主 destroy method 双重管理。

### 5.3 `FibraServiceBridge`

保持唯一公共桥接机制：

来源：当前 `fibra-spring-boot-starter/.../FibraServiceBridge.java` 已存在的签名，本变更迁入 `fibra-spring` 并继续冻结。
...
校验完整属性图
  → 构建 FibraEngine
  → 创建 FibraSpringLifecycle
  → Spring 调用 lifecycle.start()
  → engine.start() 完成初载、总 readiness 和 source 启动
  → lifecycle running = true
```

两个 source 的启动和 reconcile 次序由 engine 权威设计定义；Spring 不观察或改变该顺序。
...
任何一步失败都由 `FibraEngine.start()` 按已经完成的阶段逆序回滚：

性质：本变更冻结的目标控制流。

```text
FibraSpringLifecycle.start()
  → FibraEngine.start()
  → engine 自行执行权威回滚
```
...
- `running` 只有 engine start 全部成功后才设为 true；
- 已失败并回滚的 engine 进入终止态，不允许在同一 `ApplicationContext` 再次 start；
- Spring 随后的 context close 可以重复调用 stop，但 engine close 必须幂等。
...
```text
FibraSpringLifecycle.stop()
  → FibraEngine.close()
  → running = false
  → SmartLifecycle callback
```

`root-close-timeout` 只约束 engine 内部异步 root 关闭。engine 负责完整关闭；`stop(Runnable)` 无论成功失败都必须最终调用 callback，避免 Spring shutdown phase 永久等待。

关闭是幂等且终止性的。Spring CRaC pause/restart 不会停止本 bean，因为它不声明 pauseable；ApplicationContext 关闭后不能重新启动同一 root Scheduler。

## 8. 公共 API 与兼容策略

`fibra-spring` 的公共签名基线冻结：

- `FibraSpringLifecycle`；
- `FibraServiceBridge`。
...
- 用调用序列桩锁定正常启动和正常关闭顺序；
- Spring lifecycle 测试只验证 engine 委托、状态和 callback；load、reconcile、readiness、source 与完整回滚由 engine 模块测试；
...
- 修改根配置或 include 后，engine 自动 reconcile；
- incoming 目录出现更高版本 ZIP 后，engine 完成事务升级；
- source 与显式 deployment 竞争时最终收敛且不交叉提交；
- 关闭后无 source/reconcile 线程、旧插件实例或旧 ClassLoader 强引用。
...
无。engine 接缝、三层 Spring 模块、属性模型、目录责任、生命周期所有权、关闭语义、公开 API 和版本边界均已确定。
