...
- **JMH 版本与 depMgmt 只写在 `fibra-benchmarks/pom.xml` 本模块**，根 pom 不加 JMH 版本属性，隔离最彻底、可复现构建绝对不受影响。
...
根 `pom.xml` **唯一改动**是新增一段 profile，不动 `<modules>`：

```xml
<profiles>
  <profile>
    <id>benchmarks</id>
    <modules><module>fibra-benchmarks</module></modules>
  </profile>
</profiles>
```

效果：默认 `mvn clean verify`、`scripts/verify-reproducible-release.sh`、`scripts/verify-external-consumer.sh` 三者的模块集完全不变；基准只在显式 `mvn -Pbenchmarks ...` 时进 reactor。

### 3.3 隔离不变量（红线）

- 任何生产模块都**不得**在 `<dependency>` 中引用 `fibra-benchmarks`——否则触发 `verify-external-consumer.sh` 的"恰好 5 个生产制品"断言；
- 两个门禁脚本用显式 `-pl` 列表工作，新模块不在列表内 → 零影响；
- 根 pom 零改动（仅 profile 段），5 个生产模块 `.flattened-pom.xml` 不变，`verify-reproducible-release.sh` 的逐字节比对安全。
...
- `org.openjdk.jmh:jmh-core`（compile，本模块声明版本 1.37）；
...
1. 根 `pom.xml`：仅新增 `<profiles>/benchmarks/<modules>` 一段（不动 `<modules>`、不加 JMH 版本属性）；
2. `fibra-benchmarks/pom.xml`：parent、依赖、本模块 JMH 版本、compiler `annotationProcessorPaths`、shade（含 `AppendingTransformer`、`mainClass`、`outputFile=fibra-benchmarks.jar`、`createDependencyReducedPom=false`）；
3. `BenchmarkFixtures`（契约常量）；
4. 三个 Benchmark 类；
5. `simplelogger.properties` + `README.md`；
6. 隔离验证：`mvn clean verify`、`bash scripts/verify-reproducible-release.sh`、`bash scripts/verify-external-consumer.sh` 三者模块集与产物应与加基准前一致；再 `mvn -Pbenchmarks -pl fibra-benchmarks -am -DskipTests package` 确认基准可编译出 jar。

## 8. 风险点

1. **发布门禁**：红线是任何生产模块都不得依赖 fibra-benchmarks，且不覆盖 `deploy.skip`；
2. **可复现构建**：JMH 只写本模块 → 根 pom 仅 profile 段改动，5 个生产制品 flattened POM 与 jar 逐字节不变；
3. **enforcer 收敛**：JMH 传递依赖必要时在本模块 depMgmt 钉死；
4. **shade 元数据**：漏配 `AppendingTransformer` 会导致 `java -jar` 找不到任何基准；
5. **基准腐化**：profile 门禁使基准不随主 CI 编译，建议加一个非发布 CI 作业跑 `mvn -Pbenchmarks -pl fibra-benchmarks -am -DskipTests package` 做编译门禁；
