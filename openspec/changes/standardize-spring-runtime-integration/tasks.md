...
- [ ] 2.1 以 TDD 实现不可变 `FibraProperties`、默认值和 engine builder 一对一映射
- [ ] 2.2 以 TDD 实现创建 engine 前的完整属性校验和精确错误
- [ ] 2.3 删除旧属性、旧 getter/setter 和通用 staging，不提供兼容代码

## 3. Spring Framework 适配

- [ ] 3.1 以 TDD 实现只委托 `FibraEngine` 的 `FibraSpringLifecycle`
- [ ] 3.2 保持 `FibraServiceBridge` 显式 ServiceKey 桥接，禁止插件 bean 自动注入
- [ ] 3.3 验证 lifecycle callback、异常传播和 engine 唯一关闭所有权
...
- [ ] 5.1 Web 示例只引入 starter，staging 迁到示例命名空间
- [ ] 5.2 用真实 engine 黑盒验证 Boot wiring、reconcile 和 ApplicationContext 关闭
- [ ] 5.3 重建 spring/autoconfigure 签名，删除 starter 和旧 lifecycle 签名
- [ ] 5.4 执行全 reactor、九运行时制品、依赖边界、外部消费和可复现构建
- [ ] 5.5 更新文档、完成审查并归档 change
