...
默认结构包含 `plugin-api`、`plugin-impl`、`config` 和 `deployment`。前两者直接表达多插件共享类型的正确边界：contract 依赖 `fibra-api` provided；plugin 依赖 contract、`fibra-pf4j-api` 和 PF4J provided；私有业务库才进入 plugin ZIP `lib/`。config 保存可直接修改的 typed config 示例，deployment 负责把两份插件 ZIP 与配置打成 SHA-256 联合部署包。

### D4：生成后必须直接验证

archetype 使用 Maven Archetype Plugin 3.4.1 官方 integration-test，在 `verify` 阶段生成项目并执行其 verify；同一模块的 Failsafe 集成测试再检查 ZIP布局并由 `FibraEngine` 加载，不依赖外部脚本的执行顺序。
