...
- [x] 1.5 删除两个 loader 的 watcher 公共 API并同步 API签名，不提供兼容代码
...
- [x] 3.2 以 TDD 实现唯一 engine journal 与两个 loader 参与者的 prepare/commit/complete/rollback
- [x] 3.3 以 TDD 覆盖每个崩溃点恢复、主异常、suppressed 顺序和拒绝猜测恢复
- [x] 3.4 以真实多插件和 typed config 验证联合升级、降级及不兼容回滚
