...
public final class ClientProtocolCodec {
    ...
    private ClientMessage.Snapshot snapshot(ObjectNode payload) {
        ...
        try {
            return new ClientMessage.Snapshot(session(payload.get("session")),
                ...
                longValue(payload, "targetRevision", ErrorCode.MALFORMED_MESSAGE),
    ...
    private ClientMessage.Call call(ObjectNode payload) {
        ...
        return new ClientMessage.Call(call(payload.get("call")), text(payload, "contributionKind", ErrorCode.MALFORMED_MESSAGE),
            contributionId(payload.get("contributionId")), literal(payload.get("input")));
    }
    ...
    private LifecycleFence lifecycle(JsonNode node) {
        ...
        try {
            return new LifecycleFence(session(value.get("session")), longValue(value, "targetRevision", ErrorCode.INVALID_IDENTITY),
    ...
    private CallFence call(JsonNode node) {
        ...
        try {
            return new CallFence(session(value.get("session")), text(value, "expectedViewRevision", ErrorCode.INVALID_IDENTITY),
                longValue(value, "registrationIdentity", ErrorCode.INVALID_IDENTITY));
        } catch (IllegalArgumentException exception) {
    ...
    private List<ClientMessage.Contribution> contributions(JsonNode node) {
        ...
        for (var item : array) {
            ...
            result.add(new ClientMessage.Contribution(text(value, "contributionKind", ErrorCode.MALFORMED_MESSAGE),
                contributionId(value.get("contributionId")), longValue(value, "registrationIdentity", ErrorCode.MALFORMED_MESSAGE)));
        }
        return List.copyOf(result);
    }
    ...
    private ClientMessage.CallOutcome callOutcome(JsonNode node) {
        var value = object(node, "outcome", ErrorCode.MALFORMED_MESSAGE);
        return switch (text(value, "kind", ErrorCode.MALFORMED_MESSAGE)) {
            case "SUCCESS" -> { exactFields(value, Set.of("kind", "value"), ErrorCode.MALFORMED_MESSAGE); yield new ClientMessage.CallOutcome.Success(literal(value.get("value"))); }
    ...
    private List<ClientMessage.ExecutionObservation> executions(JsonNode node) {
        ...
        for (var item : array) {
            ...
            result.add(new ClientMessage.ExecutionObservation(longValue(value, "targetRevision", ErrorCode.MALFORMED_MESSAGE),
    ...
    private LiteralValue literal(JsonNode node) {
        ...
        if (node.isNumber()) return new LiteralValue.NumberValue(new BigDecimal(node.asString()));
        if (node instanceof ArrayNode array) {
            var result = new ArrayList<LiteralValue>(array.size());
            for (var item : array) result.add(literal(item));
            return new LiteralValue.ListValue(result);
        }
        if (node instanceof ObjectNode object) {
            var result = new HashMap<String, LiteralValue>();
            object.properties().forEach(entry -> result.put(entry.getKey(), literal(entry.getValue())));
    ...
    private ObjectNode encodeMessage(ClientMessage message) {
        ...
        switch (message) {
            case ClientMessage.Hello value -> { payload.set("identity", helloIdentity(value.identity())); payload.put("executionTarget", value.executionTarget()); strings(payload, "capabilities", value.capabilities()); }
            case ClientMessage.Welcome value -> payload.set("session", session(value.session()));
            case ClientMessage.Snapshot value -> { payload.set("session", session(value.session())); payload.put("viewRevision", value.viewRevision()); payload.put("targetRevision", value.targetRevision()); payload.put("targetDigest", value.targetDigest()); assignments(payload, value.assignments()); contributions(payload, value.contributions()); }
            ...
            case ClientMessage.LifecycleResult value -> { payload.set("lifecycle", lifecycle(value.lifecycle())); payload.set("outcome", lifecycleOutcome(value.outcome())); }
            case ClientMessage.Observed value -> { payload.set("session", session(value.session())); executions(payload, value.executions()); }
            case ClientMessage.Call value -> { callPayload(payload, value.call(), value.contributionKind(), value.contributionId()); payload.set("input", literal(value.input())); }
            case ClientMessage.CallResult value -> { callPayload(payload, value.call(), value.contributionKind(), value.contributionId()); payload.set("outcome", callOutcome(value.outcome())); }
            case ClientMessage.Detach value -> payload.set("session", session(value.session()));
        }
        return payload;
    }

    private ObjectNode helloIdentity(HelloIdentity identity) { var value = json.createObjectNode(); value.put("clientNonce", identity.clientNonce()); return value; }
    private ObjectNode session(SessionFence fence) { var value = json.createObjectNode(); value.put("hostInstanceId", fence.hostInstanceId()); value.put("clientExecutionId", fence.clientExecutionId()); return value; }
    private ObjectNode lifecycle(LifecycleFence fence) { var value = json.createObjectNode(); value.set("session", session(fence.session())); value.put("targetRevision", fence.targetRevision()); value.put("runtimeInstanceId", fence.runtimeInstanceId()); value.put("lifecycleOperationId", fence.lifecycleOperationId()); return value; }
    private ObjectNode call(CallFence fence) { var value = json.createObjectNode(); value.set("session", session(fence.session())); value.put("expectedViewRevision", fence.expectedViewRevision()); value.put("registrationIdentity", fence.registrationIdentity()); return value; }
    private void callPayload(ObjectNode payload, CallFence fence, String kind, ClientMessage.ContributionId id) { payload.set("call", call(fence)); payload.put("contributionKind", kind); payload.set("contributionId", contributionId(id)); }
    private ObjectNode contributionId(ClientMessage.ContributionId id) { var value = json.createObjectNode(); value.put("providerInstanceId", id.providerInstanceId()); value.put("localName", id.localName()); return value; }
    ...
    private ObjectNode callOutcome(ClientMessage.CallOutcome outcome) { var value = json.createObjectNode(); switch (outcome) { case ClientMessage.CallOutcome.Success success -> { value.put("kind", "SUCCESS"); value.set("value", literal(success.value())); } case ClientMessage.CallOutcome.Failed failed -> { value.put("kind", "FAILED"); value.set("failure", failure(failed.failure())); } } return value; }
    ...
    private void executions(ObjectNode payload, List<ClientMessage.ExecutionObservation> values) { var array = payload.putArray("executions"); for (var value : values) { var node = array.addObject(); node.put("targetRevision", value.targetRevision()); node.put("runtimeInstanceId", value.runtimeInstanceId()); node.put("lifecycleOperationId", value.lifecycleOperationId()); node.put("state", value.state().name()); if (value.failure() != null) node.set("failure", failure(value.failure())); } }
    private JsonNode literal(LiteralValue value) { return switch (value) { case LiteralValue.NullValue ignored -> json.getNodeFactory().nullNode(); case LiteralValue.BooleanValue scalar -> json.getNodeFactory().booleanNode(scalar.value()); case LiteralValue.StringValue scalar -> json.getNodeFactory().stringNode(scalar.value()); case LiteralValue.NumberValue scalar -> json.getNodeFactory().numberNode(scalar.value()); case LiteralValue.ListValue list -> { var array = json.createArrayNode(); list.values().forEach(item -> array.add(literal(item))); yield array; } case LiteralValue.ObjectValue object -> { var result = json.createObjectNode(); object.values().forEach((key, item) -> result.set(key, literal(item))); yield result; } }; }
