...
export interface ExecutionObservation { readonly unitTargetRevision: string; readonly runtimeInstanceId: string; readonly lifecycleOperationId: string; readonly state: "PENDING" | "ACTIVE" | "FAILED"; readonly failure?: ClientError; }
...
function lifecycle(value: JsonValue): LifecycleFence {
  ...
  exact(fence, ["session", "unitTargetRevision", "runtimeInstanceId", "lifecycleOperationId"], "INVALID_IDENTITY");
  session(fence.session);
  positiveLong(fence.unitTargetRevision, "unitTargetRevision", "INVALID_IDENTITY");
...
function assignments(value: JsonValue): void {
  for (const item of array(value, "assignments")) {
    ...
    exact(assignment, ["pluginId", "facetId", "desiredEntryId", "definitionId", "runtimeInstanceId", "unitTargetRevision", "executionTarget", "config", "entryModule", "payloadDigest", "requiredCapabilities", "resources"]);
    for (const name of ["pluginId", "facetId", "desiredEntryId", "definitionId", "runtimeInstanceId", "executionTarget", "entryModule"]) text(assignment, name);
    positiveLong(assignment.unitTargetRevision, "unitTargetRevision");
    literal(assignment.config, 4);
...
function executions(value: JsonValue): void {
  for (const item of array(value, "executions")) {
    ...
    const expected = state === "FAILED"
      ? ["unitTargetRevision", "runtimeInstanceId", "lifecycleOperationId", "state", "failure"]
      : ["unitTargetRevision", "runtimeInstanceId", "lifecycleOperationId", "state"];
    exact(execution, expected);
    if (state !== "PENDING" && state !== "ACTIVE" && state !== "FAILED") throw malformed("invalid state");
    positiveLong(execution.unitTargetRevision, "unitTargetRevision");
