...
用 `fibra-spring-boot-starter` 在 Spring Boot Web 宿主里演示托管链路：
...
- `fibra-example-spring-host-plugin`：以 `provided` scope 依赖 `Greeting` 的 executable 插件，打成标准 ZIP，由 deployment 配置创建 entry 并注册 `Greeting` 实现。
- `fibra-example-spring-host`：Spring Boot Web 宿主。控制器是宿主自己的静态 `@RestController`（不是插件贡献路由）。

宿主只注入 `FibraEngine`、root `Context` 和显式 `FibraServiceBridge`，不能取得 engine 内部 loader。`fibra-spring-boot-starter` 只负责引入自动配置；运行代码分别位于 `fibra-spring` 与 `fibra-spring-boot-autoconfigure`。
